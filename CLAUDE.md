...
`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `WHATSAPP_TOKEN`, `WHATSAPP_PHONE_ID`, `WHATSAPP_APP_SECRET` (App Secret of the Meta app — used to verify the `X-Hub-Signature-256` HMAC on every webhook delivery in [firma-meta.ts](src/lib/firma-meta.ts); **fail-closed**: if unset, `/api/webhook` rejects everything with 503, so if the bot goes silent after a deploy check this var first), `VERIFY_TOKEN` (Meta handshake **and** Supabase DB-webhook auth), `QSTASH_TOKEN`, `QSTASH_CURRENT_SIGNING_KEY`, `QSTASH_NEXT_SIGNING_KEY`, `GROQ_API_KEY` (read implicitly by `createGroq()`), `NEXT_PUBLIC_APP_URL` / `VERCEL_URL` (base URL for QStash callbacks). Stored in `.env.local`.
